    ...
    private sealed class ApplicationSuspensionManager
    {
      ...
      public ApplicationSuspensionManager()
      {
        try
        {
          ...
          WindowsRuntimeMarshal.AddEventHandler<SuspendingEventHandler>(new Func<SuspendingEventHandler, EventRegistrationToken>(current.add_Suspending), new Action<EventRegistrationToken>(current.remove_Suspending), new SuspendingEventHandler((object) this, __methodptr(OnCurrentApplicationSuspending)));
        }
        catch
        {
        }
